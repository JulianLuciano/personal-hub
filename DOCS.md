...
- `max_tokens: 8192` para ambos modelos (subido de 3000 → 4096 → 8192 — con adaptive thinking prendido en Sonnet, `max_tokens` es un techo compartido entre tokens de thinking + texto de respuesta; en respuestas largas y con muchas tablas —ej. armado de portfolio con betas/correlaciones/tramos— 4096 se quedaba corto y la respuesta se cortaba a mitad de camino).
- **Detección de corte por `max_tokens`:** Fase 2 (streaming) ahora captura `stop_reason` del evento `message_delta` y lo loggea explícito: `[ai-chat] stream done | in: X out: Y | stop_reason: max_tokens ⚠️ CORTADO POR max_tokens...` — antes solo se veían los tokens, había que adivinar si el corte fue por límite o por otra cosa. El evento SSE `done` que recibe el cliente ahora manda `truncated: true/false`; si viene truncado, `ai.js` le agrega una nota visible al final del mensaje ("⚠️ Respuesta cortada por límite de tokens") en vez de dejar la respuesta cortada sin explicación. Como esa nota se agrega a la misma variable que después se loggea a Supabase, queda persistida en el historial de la conversación, no es solo un aviso efímero en pantalla.
- **Desglose thinking vs. texto:** el log ahora muestra `out: X (thinking: Y, texto: Z)`, usando `usage.output_tokens_details.thinking_tokens` del evento `message_delta` final. `thinking_tokens` está *incluido dentro* de `output_tokens` (no es un contador aparte) — por eso `texto ≈ output_tokens - thinking_tokens`. Esto es lo que permite saber, cuando se corta por `max_tokens`, si fue el thinking el que se comió el presupuesto o si la respuesta en sí era simplemente muy larga.
- **Preview del thinking (no el razonamiento completo):** se loggea server-side (`[ai-chat] thinking preview...`, primeros 300 chars) para debugging, capturando los eventos `content_block_delta` de tipo `thinking_delta`. Importante: Anthropic solo devuelve un **resumen** del razonamiento ("summarized thinking"), nunca los tokens crudos completos — aunque factura por el total real generado internamente. No hay forma de acceder al razonamiento íntegro vía API, ni loggeándolo. Este preview nunca se manda al cliente (el filtro de "no mostrar thinking" se mantiene intacto), es solo para consola/debugging server-side.
- **Timeouts y Fase 1 convertida a streaming internamente:** Fase 1 usaba `_callAnthropicOnce` (no-streaming) con `setTimeout(90000)`. Con `Opus 5 a effort:high` en preguntas largas/complejas esto saltaba: al ser no-streaming, no llega ningún byte hasta que Anthropic termina de generar la respuesta entera, y sin actividad en el socket el `setTimeout` de Node dispara por inactividad aunque el modelo esté trabajando normalmente. Se agregó `_callAnthropicStreamOnce` / `callAnthropicStreaming()`, que hace la misma llamada pero con `stream:true` y reconstruye el mismo shape `{stop_reason, content, usage}` que devolvía la versión no-streaming (para no tocar la lógica del loop). Con streaming, Anthropic manda eventos `ping` periódicos mientras genera, así que el socket nunca queda inactivo de verdad — el timeout deja de depender de adivinar cuánto puede tardar el modelo en pensar. Igual se dejó `setTimeout(240000)` como red de seguridad ante una falla de red real (no un modelo pensando). Fase 2 (que ya era streaming) también tiene `setTimeout(240000)` por consistencia, aunque ahí el riesgo siempre fue menor.
- **Reconstrucción de `thinking` con `signature`:** si el modelo hace thinking + tool_use en el mismo turno, Anthropic espera el bloque de thinking íntegro (con su `signature`) al reenviar el historial en la siguiente vuelta del loop — mandar uno incompleto puede tirar 400. `_callAnthropicStreamOnce` acumula `thinking_delta` y `signature_delta` completos (no solo un preview) para reconstruir el bloque fielmente antes de empujarlo a `loopMessages`.
- **Opus bajado de `effort:high` a `effort:medium`:** el disparador real del timeout no era un bug de código sino Opus 5 tardando genuinamente más de 90s en terminar de pensar a `high` en preguntas grandes y multi-parte. Streaming resuelve el *timeout artificial*, pero no acorta el tiempo real que tarda el modelo en pensar — para eso, la palanca es `effort`. `medium` es la recomendación de Anthropic como "cost-saving step-down" manteniendo buena calidad; se puede reevaluar si en la práctica hace falta más profundidad para preguntas puntuales.
