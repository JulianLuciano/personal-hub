...
| Chat falla con 429 o 529 y no reintenta | `routes/ai-server.js` → `callAnthropic()`/`_callAnthropicOnce()` (OCR, token-diag) o `callAnthropicStreaming()`/`_callAnthropicStreamOnce()` (Fase 1 del chat principal) — ambos tienen retry automático |
| Chat interactivo se corta a mitad de respuesta | `routes/ai-server.js` → revisar log `[ai-chat] stream done \| ... \| stop_reason: ...`. Si es `max_tokens`, revisar `usage.thinking_tokens` para ver si el thinking se comió el presupuesto; si es alto, considerar bajar `effort` o subir `max_tokens` (hoy 8192) |
...
- **Preview del thinking (no el razonamiento completo):** loggeado server-side (`[ai-chat] thinking preview...`, primeros 300 chars), capturando `content_block_delta` de tipo `thinking_delta`. Anthropic solo devuelve un **resumen** ("summarized thinking"), nunca los tokens crudos completos, aunque factura por el total real generado internamente — no hay forma de acceder al razonamiento íntegro vía API. Nunca se manda al cliente (el filtro de "no mostrar thinking" se mantiene intacto).
- **Fase 1 convertida a streaming internamente** (`_callAnthropicStreamOnce` / `callAnthropicStreaming()`, reemplaza `_callAnthropicOnce` para esta fase): la versión no-streaming vieja no recibía ningún byte hasta que Anthropic terminaba de generar la respuesta entera, así que con Opus a effort alto en preguntas largas el `setTimeout` de Node disparaba por inactividad de socket aunque el modelo estuviera funcionando bien. Con streaming, Anthropic manda `ping` periódicos, el socket nunca queda inactivo de verdad, y el timeout deja de depender de adivinar cuánto tarda el modelo en pensar. Reconstruye el mismo shape `{stop_reason, content, usage}` que la versión vieja para no tocar la lógica del loop.
- **Timeouts:** ambas fases en `setTimeout(240000)` (240s, subido de 90s) — sirve como red de seguridad ante una falla de red real, no como mecanismo principal contra "el modelo tarda en pensar" (eso ya lo resuelve el streaming de Fase 1).
- **Reconstrucción de `thinking` con `signature`:** si el modelo hace thinking + tool_use en el mismo turno, Anthropic espera el bloque de thinking íntegro (con `signature`) al reenviar el historial en la siguiente vuelta del loop — mandar uno incompleto puede tirar 400. `_callAnthropicStreamOnce` acumula `thinking_delta` y `signature_delta` completos (no solo un preview) antes de empujarlo a `loopMessages`.
...
- `aiLoadHistory(searchQuery?)` — fetcha las últimas 15 conversaciones de `/api/ai-conversations`, o resultados de búsqueda de `/api/ai-conversations/search?q=...` si se pasa un query. Lazy: solo carga la primera vez; se resetea con `aiHistoryLoaded = false` cada vez que se loggea un mensaje nuevo.
- `aiShowHistoryView()` — inyecta un input de búsqueda con debounce de 320ms la primera vez que se abre el historial. La búsqueda es fulltext sobre el cuerpo de los mensajes y los títulos de conversación.
...
- `aiComputeSignals()` — lee `liveData` y `window._macroData` y devuelve un objeto con todas las señales activas: `vixHigh` (VIX > 28), `equityDayPct` + `equityDrop` (equity ponderado ≤ −3%) + `equityRise` (≥ +3%), `bigMovers` (posiciones con |dayPct| ≥ 5%), `deepRed` (posiciones con pnlUSD% ≤ −10%). Excluye RENT_DEPOSIT, EMERGENCY_FUND y GBP_LIQUID.
- `aiRenderAlerts()` — renderiza el banner `#aiAlertBanner` con las alertas activas. Si no hay alertas activas, oculta el div. Si hay, muestra un header colapsable "› Alertas [N]" con chevron animado (igual estilo que tool-log), pills clickeables por cada alerta, y botón × para cerrar por sesión.
- `_alertsDismissed` — flag de sesión: si el usuario cierra el banner con ×, no vuelve a aparecer hasta recargar la página.
- `_alertsExpanded` — estado de colapso persistente entre aperturas del modal en la misma sesión.
- `_alertToPrompt(alert)` — mapea cada alerta a una pregunta contextual pre-llenada en el input al tocar la pill.
...
**Safeguard: cashflows backdated en el cálculo de "Rendimiento del día" (agosto 2026):**
- **Bug encontrado:** el 19 ago 2026 el briefing mostró "Rendimiento del día" de +$921/+£579 cuando el movimiento real de mercado era de solo ~+$241/+£79. Causa: una transacción `WITHDRAWAL` de $679.69 (EMERGENCY_FUND) tenía `date`/`created_at` de hoy, pero la plata había salido de la cuenta días antes — el registro de hoy era solo un ajuste contable retroactivo, no una salida de caja real. El cálculo de `netCFusd` (Modified Dietz, `dayChangeUSD = totalUSD - snap24h.total_usd - netCFusd`) no tiene forma de distinguir "cashflow real de hoy" de "ajuste contable fechado hoy", así que sumó de vuelta los $680 como si hubieran salido físicamente hoy, inflando el rendimiento reportado.
- **Fix (ver `netCFusdForDayReturn` en `routes/ai-server.js`, justo antes del cálculo de `dayChangeUSD`):** cuando `netCFusd !== 0`, se comparan dos lecturas independientes del movimiento del día:
  - `rawDiffUSD` = `totalUSD - snap24h.total_usd` (top-down, snapshot a snapshot, sin ajustar por cashflow)
  - `equityDiffUSD` = Σ por posición `(priceLatest - price24hAgo) × qty` (bottom-up, solo precio, nunca toca cash/fiat)
  
  Si `|rawDiffUSD - equityDiffUSD| <= tolerancia` (£2 o su equivalente en USD según `fxRate` del momento), se asume que el cashflow fechado hoy no movió plata real hoy → se excluye del cálculo de rendimiento (`netCFusdForDayReturn = 0`). Si no matchea, se aplica como antes (cashflow real).
  - El `netCFusd` real (sin tocar) se sigue mostrando en el prompt (`day_cashflow_usd/gbp`) para que la IA pueda mencionar el movimiento como contexto — solo cambia si se usa o no para calcular el rendimiento.
  - Se agregó `day_cashflow_note` al prompt para que la IA sepa explícitamente si el movimiento fue tratado como ajuste contable o como cashflow real, y se le pasó la fecha exacta de la transacción (antes solo tenía `type, amount_usd`) para que no invente "ayer"/"hoy" al redactar.
- **Limitación conocida (documentada en el código, no resuelta):** es una heurística sobre el movimiento agregado del equity, no un flag explícito en la transacción. Dos cashflows el mismo día que se cancelan en magnitud (ej. un depósito real de +$500 y un ajuste retroactivo de -$680 el mismo día) pueden hacer que el check falle en cualquier dirección — el residual quedaría pareciendo "real" o "backdated" por casualidad numérica, no por la naturaleza real de cada movimiento.
- **TODO / fix de fondo:** el arreglo durable es un campo explícito en `transactions` (ej. `is_backdated boolean` o `cash_effective_date date`) que indique si una fila representa un movimiento de caja del día en que está fechada o una corrección retroactiva. Mientras no exista ese campo, el safeguard de arriba es la mejor aproximación automática disponible, pero no es infalible en el caso patológico de arriba.
- **Fix #2 (20 ago 2026, con datos reales, no simulados):** la primera versión del safeguard comparaba `rawDiff` solo contra `equityDiffUSD` (posiciones invertidas), sin tocar el cash. Validando con snapshots reales (18→20 ago, ventana sin ningún cashflow) esto dio un residual de **$33.15** — 14x la tolerancia de £2/$2.72 — que hubiera hecho fallar el check para cualquier cashflow real que cayera en un día con movimiento de FX parecido. Causa aislada: el cash en libras (£4,868, sin cambios de balance) se revaluó de $6,587.86 a $6,621.94 solo por el movimiento del `fx_rate` entre esos dos snapshots — un swing de $34.08, casi todo el residual. **Fix:** se agregó `cashFXDiffUSD` al cálculo, que revalúa el balance de cash en GBP/ARS *actual* a ambos tipos de cambio (aislando el efecto FX puro, sin mezclar cambios reales de balance — eso queda deliberadamente en el residual, para no enmascarar transferencias reales). Con el fix, el mismo caso real dio residual de **$0.93**, dentro de tolerancia. Ver comentario extenso en el código (`routes/ai-server.js`, bloque `cashFXDiffUSD`) para el detalle completo del caso y por qué se usa el balance "now" en vez de promediar ambos extremos.
...
El endpoint implementa dos fases:

**Fase 1 — Tool loop (streaming interno, sin exponer progreso al cliente):** resuelve todas las tool calls server-side antes de responder al cliente. Usa `callAnthropicStreaming()` en vez de la versión no-streaming vieja — el motivo es robustez de conexión (ver sección de Timeouts más abajo), no mostrar progreso incremental; el cliente sigue sin ver nada de esta fase hasta el `tool_log` final.
```
...
- **Logs en Railway:** entrada (model, tamaño system prompt, user msg), cada tool (nombre, input, elapsed ms), y cierre del stream (tokens in/out, cache_read si aplica).
...
- `years` (requerido, 1–40) o `months` (1–480, tiene precedencia sobre `years`)
- `monthly_contribution_gbp` (default 950)
- `annual_bonus_gbp` (default 8000)
- `include_rsu` (default true)
- `rsu_per_vest_override` (opcional): override manual del valor neto por vest en GBP
- `target_gbp` (opcional): si se especifica, devuelve probabilidad de alcanzarlo al final del horizonte
...
**Tool: `run_montecarlo_target`** — `executeRunMontecarloTarget(input)` (async)

**Monte Carlo inverso:** dado un target en GBP, devuelve la distribución de *tiempo* para alcanzarlo. Usar cuando el usuario pregunta "¿cuándo llego a £X?" o "¿en cuánto tiempo tengo £X?".

- Misma lógica de simulación que `run_montecarlo` (mismo capital inicial, RSU, cash flows).
- Para cada simulación: guarda el **primer mes en que el portfolio cruza `target_gbp`** (o Infinity si no llega dentro del horizonte máximo).
- **Output:** `months_to_target` (p10/p25/median/p75/p90 en formato legible, ej. "2 años y 3 meses"), `prob_by_horizon` (probabilidad de llegar en 1/2/3/5/10 años), `pct_reached` (% de simulaciones que llegan), nota con cuántas no llegaron.
- `max_horizon_months` (default 180 = 15 años, max 240).

**Parámetros del tool `run_montecarlo_target`:**
- `target_gbp` (requerido)
- `max_horizon_months` (default 180)
- `monthly_contribution_gbp`, `annual_bonus_gbp`, `include_rsu`, `rsu_per_vest_override`, `scenario`, `initial_capital_gbp` — igual que `run_montecarlo`.
...
- `GET /api/ai-conversations/search?q=...` — búsqueda fulltext por `ilike` en `ai_messages.content` y `ai_conversations.title`. Devuelve array de conversaciones (mismo formato que GET `/api/ai-conversations`) ordenadas por fecha desc. Hasta 50 conversaciones únicas.
...
- `GET /api/briefing-context` — arma el system prompt completo para el briefing diario. Autosuficiente: no depende de ningún cache en memoria (funciona aunque la app no haya sido abierta en todo el día). Fetcha en paralelo:
  ...
  **Secciones del system prompt generado:**
  ...
  - **Estructura del output** (instrucción de formato en el prompt, no un dato): 3 secciones fijas — `Resumen del día` (tabla USD/GBP + prosa condicional por gap FX), `Qué importa hoy` (posiciones por impacto nominal, sin aclarar el criterio en el texto; macro solo si es relevante), `Contexto y oportunidades` (una sola sección fusionada — antes eran dos, "Contexto" y "Observación concreta", que se pisaban repitiendo el mismo ticker). `deployable_cash` solo se menciona si es > $0 y hay algo concreto que hacer con eso.
  - **Sección 3 rediseñada (ago 2026)** — el diseño original forzaba "1 línea de estado siempre" con % de RSU_META y días para el vest, sin condición, así que aparecía todos los días aunque no hubiera novedad. Ahora:
    - RSU_META/vest solo se menciona si: (a) la posición supera 30% del equity investible, (b) el vest está a menos de 30 días, (c) el vest es hoy/mañana, o (d) el % cambió de forma significativa vs. el briefing de ayer (cruzó un umbral redondo).
    - **Chequeo de repetición contra ayer:** como esas condiciones se cumplen de forma persistente día tras día mientras la situación no cambie (ej. 35% sigue siendo >30% mañana y pasado), el prompt le pide al modelo comparar explícitamente contra el briefing de ayer (que ya viaja completo en el prompt) — si ya dijo básicamente lo mismo, NO lo repite hoy, aunque la condición siga técnicamente cumplida. Vuelve a mencionarse solo si: cruzó un umbral redondo distinto, el vest bajó a un hito más cercano, o no se mencionó en los últimos 3-4 días (salvaguarda para que el dato no desaparezca del radar por semanas si la concentración sigue alta).
...
server.js — loop agentic + streaming
  ...
  Fase 1 (tool loop, streaming interno vía callAnthropicStreaming — ver detalle en la sección de ai.js más arriba; el cliente no ve nada de esta fase, el streaming acá es solo por robustez de conexión):
...
- "¿Cuánto tendré en 3 años en escenario bear?" → `years: 3, scenario: bear`

**`run_montecarlo_target`** — cuando el usuario pregunta cuánto tiempo tarda en llegar a un monto:
- "¿Cuándo llego a £100k?" → `target_gbp: 100000`
- "¿En cuánto tiempo tengo £200k?" → `target_gbp: 200000`
...
POST /api/ai-chat → server.js
    ↓
Loop agentic (Fase 1 streaming interno, max 6 iteraciones — MAX_TOOL_ITERATIONS):
